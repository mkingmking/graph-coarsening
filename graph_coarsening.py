    ...
    for route in routes:
        ...
        for i in range(len(route) - 1):
            ...
            if service_start_time_at_to_node > to_node.l:
                ...
                # print(f"  Violation: Node {to_node_id} arrived too late. Expected by {to_node.l:.2f}, arrived at {arrival_time_at_to_node:.2f}, service start {service_start_time_at_to_node:.2f}")
...
class SpatioTemporalGraphCoarsener:
    ...
    def coarsen(self) -> tuple[Graph, list]:
        ...
        level = 0
        print(f"--- Starting Coarsening ---")
        print(f"Initial graph size: {len(G_prime.nodes)} nodes, {len(G_prime.edges)} edges")
        ...
        while len(G_prime.nodes) > self.P * n_0:
            ...
            if not M:
                print("No feasible merges found at this level. Breaking coarsening loop.")
                break # No more merges possible

            print(f"  Found {len(M)} merges at this level.")
            ...
            print(f"  Current graph size: {len(G_prime.nodes)} nodes, {len(G_prime.edges)} edges")
    ...
    def inflate_route(self, coarsened_routes: list) -> list:
        ...
        print(f"Initial coarsened routes: {coarsened_routes}")
        ...
        for route_idx, coarsened_route in enumerate(coarsened_routes):
            ...
            print(f"  Inflating Route {route_idx + 1}: {inflated_route}")
            ...
            for super_node_id, node_i_id, node_j_id, pi_order in reversed(self.merge_layers):
                ...
                if replaced_this_layer:
                    print(f"    Inflated {super_node_id} to {ordered_pair}. Current Route {route_idx + 1}: {inflated_route}")
                
            all_inflated_routes.append(inflated_route)
            print(f"  Route {route_idx + 1} Inflation Finished. Final: {inflated_route}")
            
        print(f"--- Inflation Finished ---")
        print(f"Final inflated routes: {all_inflated_routes}")
