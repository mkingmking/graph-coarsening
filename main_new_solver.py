...
from cvrptw_coarsener import CVRPTWCoarsener
...
def run_solver_pipeline(graph: Graph, depot_id: str, vehicle_capacity: float, solver_name: str, coarsener: SpatioTemporalGraphCoarsener = None):
    ...
    if solver_name in ('Greedy', 'Savings'):
        ...
        if coarsener:
            ...
            metrics = calculate_route_metrics(coarsener.initial_graph, routes, depot_id, vehicle_capacity)
...
def run_inflated_solvers(coarsener: CVRPTWCoarsener, coarsened_graph: Graph, depot_id: str, capacity: float, initial_graph: Graph):
    """
    Runs solvers on the coarsened graph and then inflates the solutions.
    """
    logger.info("\n" + "="*30 + " Inflated Solvers " + "="*30)
    results = {}
    for name in ["Greedy, savings"]: # Add other solvers here as needed
        
        # 1. Run the solver on the coarsened graph
        coarsened_routes, _ = run_solver_pipeline(coarsened_graph, depot_id, capacity, name)
        
        # 2. Inflate the solution from the coarsened graph back to the original graph
        # This is the critical step that was missing before.
        inflated_routes = coarsener.inflate_route(coarsened_routes)
        
        # 3. Calculate metrics for the inflated routes on the *initial* graph
        # The previous error occurred because coarsened_routes were being passed here.
        inflated_metrics = calculate_route_metrics(initial_graph, inflated_routes, depot_id, capacity)

        results[f"{name}_inflated"] = {
            "routes": inflated_routes,
            "metrics": inflated_metrics
        }
        ...
        filename = f"{name}{count}"
        visualize_routes(initial_graph, inflated_routes, depot_id, "coarsened Solution", filename = "coarsened Solution" + filename)
...
def process_file(csv_file_path: str) -> dict:
    ...
    coarsener = CVRPTWCoarsener(graph=graph, depot_id=depot_id, vehicle_capacity=capacity, wc=0.4, wd=0.2, wt=0.4)
    coarsened_graph, merge_layers = coarsener.coarsen(coarsener.initial_graph)
